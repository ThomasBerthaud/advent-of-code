...
traceWire(wire1, (x, y, dist) => (coordinates[`${x}|${y}`] = dist));
traceWire(wire2, (x, y, dist) => {
  if (coordinates[`${x}|${y}`]) {
    intersections.push({ dist: coordinates[`${x}|${y}`], dist2: dist });
  }
});
...
console.log(bestPoint, bestPoint.dist + bestPoint.dist2);

function traceWire(wire, callback) {
  let x = 0,
    y = 0;
  let dist = 0;
  wire.forEach(movement => {
    const direction = movement[0];
    const steps = movement.slice(1);
